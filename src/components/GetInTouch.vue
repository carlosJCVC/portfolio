<template>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
        <div v-for="field in formFields" :key="field.name" class="relative">
            <input v-if="field.type !== 'textarea'" @input="validateField(field.name)" :type="field.type"
            ...
            <textarea v-else :id="field.name" v-model="form[field.name]" @input="validateField(field.name)"
                :placeholder="field.placeholder" :class="{ 'border-red-500': v$[field.name].$error }"
                class="w-full p-4 bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-700 rounded-lg focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-400 text-gray-800 dark:text-white transition duration-300"
                rows="4"></textarea>
            ...
            <p v-if="v$[field.name].$error" class="mt-1 text-sm text-red-600">{{ v$[field.name].$errors[0].$message }}
            </p>
        </div>
        ...
        <p v-if="v$.terms.$error" class="mt-1 text-sm text-red-600">{{ v$.terms.$errors[0].$message }}</p>

        <button type="submit" :disabled="v$.$invalid || isSubmitting"
