<template>
  ...
  :class="{ 'bg-white shadow-md': scrolled && !isDarkMode, 'bg-gray-900 shadow-md': scrolled && isDarkMode, 'bg-transparent': !isDarkMode}">
        ...
        <div class="hidden md:block">
          <div class="ml-10 flex items-baseline space-x-4">
            <a v-for="item in navItems" :key="item.href" :href="item.href"
              ...
              :class="[scrolled && !isDarkMode ? 'text-gray-800 hover:text-indigo-600' : 'text-white hover:text-indigo-200']">
              {{ item.text }}
            </a>
          </div>
        </div>
    ...
    <div class="md:hidden" :class="{ 'hidden': !mobileMenuOpen }" id="mobile-menu">
      <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
        <a v-for="item in navItems" :key="item.href" :href="item.href"
          ...
          :class="[scrolled && !isDarkMode ? 'text-gray-700 hover:text-indigo-600 hover:bg-indigo-50' : 'text-white hover:text-indigo-200 hover:bg-gray-800']"
