<template>
  <div class="h-full flex bg-[#1e1e1e] text-gray-300 font-sans select-none">
    ...
    <div class="w-12 bg-[#333333] flex flex-col items-center py-4 gap-6 border-r border-[#1e1e1e]">
      <i class="far fa-copy text-2xl text-white cursor-pointer opacity-100 border-l-2 border-white pl-3 -ml-3"></i>
      <i class="fas fa-search text-2xl text-gray-500 hover:text-white cursor-pointer transition-colors"></i>
      <i class="fas fa-code-branch text-2xl text-gray-500 hover:text-white cursor-pointer transition-colors"></i>
      <i class="far fa-envelope text-2xl text-gray-500 hover:text-white cursor-pointer transition-colors"></i>
      <div class="flex-grow"></div>
      <i class="fas fa-cog text-2xl text-gray-500 hover:text-white cursor-pointer transition-colors"></i>
    </div>

    <!-- Sidebar (Explorer) -->
    <div class="w-60 bg-[#252526] flex flex-col text-sm border-r border-[#1e1e1e]">
      <div class="px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-wider flex justify-between items-center">
        <span>Explorer</span>
      ...
      <!-- Project Folder -->
      <div class="mt-2">
        <div class="px-2 py-1 flex items-center gap-1 hover:bg-[#2a2d2e] cursor-pointer text-blue-400 font-bold">
    ...
    <div class="flex-grow flex flex-col bg-[#1e1e1e]">
      ...
      <div class="flex bg-[#252526] overflow-x-auto">
        <div 
          ...
          :class="activeFile.name === file.name ? 'bg-[#1e1e1e] text-white border-t-2 border-t-blue-500' : 'text-gray-500 hover:bg-[#2d2d2d]'"
          @click="openFile(file)"
        >
      ...
      <!-- Breadcrumbs -->
      <div class="h-6 bg-[#1e1e1e] flex items-center px-4 text-xs text-gray-500 gap-2 border-b border-[#2d2d2d]">
        <span>portfolio</span>
      ...
      <div class="flex-grow flex overflow-auto font-mono text-sm relative">
        ...
        <!-- Code -->
        <div class="flex-grow p-4 text-gray-300 leading-6 whitespace-pre" v-html="activeFile.content"></div>
      </div>
