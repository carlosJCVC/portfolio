  ...
  >
    <div v-for="(line, i) in history" :key="i" class="mb-1">
      <span class="text-blue-400 font-bold">visitor@carlos-dev</span>:<span class="text-white"
        >{{ line.path || '~' }}</span
      >$ {{ line.cmd }}
      <div v-if="line.output" class="text-gray-300 whitespace-pre-wrap mt-1" v-html="line.output"></div>
    </div>

    <div class="flex items-center">
      <span class="text-blue-400 font-bold">visitor@carlos-dev</span>:<span class="text-white"
        >{{ currentPathString }}</span
      >$&nbsp;
      <input
        ref="inputRef"
        v-model="currentCmd"
...
const fileSystem = computed(() => {
  return {
    'projects': {
      ...
      children: projects.reduce((acc, p) => {
        acc[p.title.replace(/\s+/g, '-').toLowerCase()] = {
          type: 'dir',
          data: p,
          children: p.files.reduce((fAcc, f) => {
            fAcc[f.name] = { type: 'file', data: f, project: p }
            return fAcc
          }, {})
        }
        return acc
    ...
    'about.txt': { type: 'file', content: 'I am Carlos Veizaga, a Full Stack Developer passionate about building scalable solutions.' },
    'skills.txt': { type: 'file', content: 'Vue.js, Node.js, Tailwind CSS, DevOps, and more...' },
    'contact.txt': { type: 'file', content: 'Email: carlos.veizaga.c@gmail.com' }
  }
})
...
const execute = async () => {
  ...
  switch (cmd) {
    ...
    case 'sudo':
      output = 'Permission denied: You are not in the sudoers file. This incident will be reported to Santa Claus.'
      break
